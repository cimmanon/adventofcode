...
checkSum :: String -> Int
checkSum x = sum $ map rowSum $ map (splitOn "\t") $ lines x
...
checkRem :: String -> Int
checkRem x = sum $ map (rowRem . map read) $ map (splitOn "\t") $ lines x

rowRem :: [Int] -> Int
rowRem (x:xs) = maybe (rowRem xs) (\y -> max x y `div` min x y) $ find (\y -> (max x y `rem` min x y)  == 0) xs
rowRem _ = 0
